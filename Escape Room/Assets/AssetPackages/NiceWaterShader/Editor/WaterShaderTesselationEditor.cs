...
public class WaterShaderTesselationEditor : ShaderGUI {
	...
	public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
	{
		...
		else
		{
			...
			//CustomFont
			Font customFont = AssetDatabase.LoadAssetAtPath(GetAssetPath()+"Interface/Simple.ttf", typeof(Font)) as Font;
			//TitleImage
			Texture titleTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Title.png", typeof(Texture2D)) as Texture2D;
			//ColorsImage
			Texture colorTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Colors.png", typeof(Texture2D)) as Texture2D;
			//FoamImage
			Texture foamTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Foams.png", typeof(Texture2D)) as Texture2D;
			//ReflectionsImage
			Texture reflectionsTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Reflections.png", typeof(Texture2D)) as Texture2D;
			//WavesImage
			Texture wavesTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Waves.png", typeof(Texture2D)) as Texture2D;
			//TexturesImage
			Texture texturesTex = AssetDatabase.LoadAssetAtPath(GetAssetPath() + "Interface/Textures.png", typeof(Texture2D)) as Texture2D;
			#endregion
			#region TitleImage
			...
			//Foldouts
			GUIStyle foldoutTitleStyle = new GUIStyle("Foldout");
			...
			//Titles
			GUIStyle titleColorStyle = new GUIStyle("label") { alignment = TextAnchor.MiddleCenter, fontSize = 13, fontStyle = FontStyle.Bold };
			#endregion
			...
			if (colorFoldout)
			{
				...
				normalsIntensity = EditorGUILayout.Slider("Normals Intensity", targetMat.GetFloat("_NormalsIntensity"), 0, 1);
			...
			if (EditorGUI.EndChangeCheck() || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed"))
			{
				...
				if (isSpecularNormalIntensity) { targetMat.SetFloat("_isSpecularNormalIntensity", 1); } else { targetMat.SetFloat("_isSpecularNormalIntensity", 0); }
				...
				if (lights) { targetMat.SetFloat("_Lights", 1); } else { targetMat.SetFloat("_Lights", 0); }
				targetMat.SetFloat("_LightColorIntensity", lightColorIntensity);

				//Foams
				targetMat.SetColor("_FoamColor", foamColor);
				if (mainFoamPower) { targetMat.SetFloat("_MainFoamPower", 1); } else { targetMat.SetFloat("_MainFoamPower", 0); }
				...
				//VertexOffset
				if (vertexOffset) { targetMat.SetFloat("_VertexOffset", 1); } else { targetMat.SetFloat("_VertexOffset", 0); }
				...
				//RealtimeReflections
				if (realtimeReflections) { targetMat.SetFloat("_RealtimeReflections", 1); } else { targetMat.SetFloat("_RealtimeReflections", 0); }
