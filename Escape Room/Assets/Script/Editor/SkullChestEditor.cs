...
[CustomEditor(typeof(Interactive_SkullLock))]
[CanEditMultipleObjects]
public class SkullChestEditor : Editor
{
    ...
    private void SetFieldCondition()
    {
        ...
        ShowOnEnum("Type", "Advanced", "Three"); //Varibale "Three" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
        ShowOnEnum("Type", "Advanced", "Four"); //Varibale "Four" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
        ShowOnEnum("Type", "Advanced", "Five"); //Varibale "Five" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
        ShowOnEnum("Type", "Advanced", "Six"); //Varibale "Six" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Modifications to update editor scene in realtime

        //Set SkullLock script as target to access its public varibles and functions
        var SkullLockScript = target as Interactive_SkullLock;
        //Create the needed variables
        SkullLockScript.AssignVariables();
        SkullLockScript.CreateLists();
        //Is the chest in Advanced or Standard state?
        if (SkullLockScript.Type == Interactive_SkullLock.ChestTypeEnum.Standard && IsStandard == false)
        {
            SkullLockScript.SwitchChestType();
            //Store the state
            IsStandard = true;
            IsAdvanced = false;
        }

        if (SkullLockScript.Type == Interactive_SkullLock.ChestTypeEnum.Advanced && IsAdvanced == false)
        {
            SkullLockScript.SwitchChestType();
            //Store the state
            IsAdvanced = true;
