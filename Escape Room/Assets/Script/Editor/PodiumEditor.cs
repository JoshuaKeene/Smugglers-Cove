...
[CustomEditor(typeof(Interactive_Podium))]
...
public class PodiumEditor : Editor
{
    ...
    private void SetFieldCondition()
    {
        ...
        ShowOnEnum("Type", "One", "Linked_Podium"); //Varibale "Three" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
        ShowOnEnum("Type", "Two", "Linked_Podium_A"); //Varibale "Four" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
        ShowOnEnum("Type", "Two", "Linked_Podium_B"); //Varibale "Five" is only visible when "Type" == "Advanced" in the Interactive_SkullLock script
    ...
    public override void OnInspectorGUI()
    {
        ...
        //Modification - Josh K
        var PodiumScript = target as Interactive_Podium;
        if (PodiumScript.Type == Interactive_Podium.TypeEnum.One)
        {
            if (GUILayout.Button("Reset"))
            {
                PodiumScript.Linked_Podium = null;
            }
        }
        if (PodiumScript.Type == Interactive_Podium.TypeEnum.Two)
        {
            if (GUILayout.Button("Reset"))
            {
                PodiumScript.Linked_Podium_A = null;
                PodiumScript.Linked_Podium_B = null;
            }
        }
